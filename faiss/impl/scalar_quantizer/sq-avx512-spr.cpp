...
struct QuantizerBF16<SIMDLevel::AVX512_SPR> : QuantizerBF16<SIMDLevel::AVX512> {
    ...
    // Override encode/decode to use VCVTNEPS2BF16 and AVX-512 decode.
    // This TU is compiled with -mavx512bf16, so the #ifdef __AVX512BF16__
    // path inside encode_bf16_simd is active.
 ...
 *
 * Delegate to AVX512 implementations. The AVX512 partial
 * specializations accept any Quantizer/Similarity types,
 * so SPR-level quantizers and similarities work directly.
 **********************************************************/
 ...
 *
 * VDPBF16PS (_mm512_dpbf16_ps) computes dot products directly
 * on bf16 data — 32 bf16 pairs per instruction, no decode step.
 *
 * For L2, we fuse both dot(q,c) and dot(c,c) into a single
 * pass over the code vector to avoid reading it twice.
 **********************************************************/

/// Inner product: dot(a, b) in bf16 domain.
...
/// Fused asymmetric L2: computes -2·dot(q,c) + ||c||² in one pass.
/// Caller adds query_norm_sq to get ||q - c||².
/// Single pass over code avoids reading it twice (cache-friendly).
static FAISS_ALWAYS_INLINE float bf16_L2_asymmetric_fused(
...
/// Fused symmetric L2: computes ||a - b||² in one pass.
/// Computes dot(a,a) - 2·dot(a,b) + dot(b,b) reading each vector once.
static FAISS_ALWAYS_INLINE float bf16_L2_symmetric_fused(
        const uint16_t* a,
        const uint16_t* b,
        size_t d) {
 ...
 *
 * Encodes the query to bf16 once, then computes dot(q, c)
 * directly in bf16 via VDPBF16PS — no per-element decode.
 **********************************************************/

struct DCBF16_IP_SPR : SQDistanceComputer {
    using Sim = SimilarityIP<SIMDLevel::AVX512_SPR>;

    size_t d;
    std::vector<uint16_t> query_bf16;

    DCBF16_IP_SPR(size_t d, const std::vector<float>&) : d(d), query_bf16(d) {}
 ...
 *
 * Uses ||q - c||² = ||q||² - 2·dot(q,c) + ||c||²
 *
 * query_norm_sq is precomputed once per query in set_query().
 * query_to_code() fuses dot(q,c) and ||c||² into a single
 * pass over the code vector (one load of each cache line).
 * symmetric_dis() fuses all three terms in one pass over
 * both vectors.
 **********************************************************/

struct DCBF16_L2_SPR : SQDistanceComputer {
    ...
    DCBF16_L2_SPR(size_t d, const std::vector<float>&)
    ...
    float query_to_code(const uint8_t* code) const final {
        return query_norm_sq +
                bf16_L2_asymmetric_fused(
                        query_bf16.data(), (const uint16_t*)code, d);
    }

    float symmetric_dis(idx_t i, idx_t j) override {
        return bf16_L2_symmetric_fused(
                (const uint16_t*)(codes + i * code_size),
                (const uint16_t*)(codes + j * code_size),
                d);
    }
...
/**********************************************************
 * Wire BF16 specializations into the dispatch template.
 *
 * These explicit specializations override the generic
 * DCTemplate<Q, S, AVX512_SPR> (which inherits from AVX512)
 * for just the BF16 quantizer + IP/L2 similarity combinations.
 **********************************************************/
...
struct DCTemplate<
        ...
        SIMDLevel::AVX512_SPR> : DCBF16_IP_SPR {
    using Sim = SimilarityIP<SIMDLevel::AVX512_SPR>;
    using DCBF16_IP_SPR::DCBF16_IP_SPR;
};
...
struct DCTemplate<
        ...
        SIMDLevel::AVX512_SPR> : DCBF16_L2_SPR {
    using Sim = SimilarityL2<SIMDLevel::AVX512_SPR>;
    using DCBF16_L2_SPR::DCBF16_L2_SPR;
};
